...
export default function LoginPage() {
  ...
  const { sendOtp, verifyOtp, resendOtp, isAuthenticated, isLoading } = useAuth();
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!/^\d{4,6}$/.test(otp)) {
      setFormError('Enter the OTP received on your mobile');
      return;
    }
    setFormError(null);
    setSubmitting(true);
        ...
        {step === 'mobile' ? (
            ...
            <form onSubmit={handleSendOtp}>
              <div className={styles.formGroup}>
                <label className={styles.label}>
                  Mobile Number<span style={{ color: 'white', marginLeft: 4 }}>*</span>
                </label>
                <input
                  type="tel"
          ...
          <>
            ...
            <form onSubmit={handleVerifyOtp}>
              <div className={styles.formGroup}>
                <label className={styles.label}>
                  OTP<span style={{ color: 'white', marginLeft: 4 }}>*</span>
                </label>
                <input
                  type="text"
                  inputMode="numeric"
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <button
                type="button"
                className={styles.googleButton}
                onClick={() => { setStep('mobile'); setOtp(''); setFormError(null); }}
              >
                <span className={styles.googleButtonText}>← Change Number</span>
              </button>
            </div>
          </>
        )}
